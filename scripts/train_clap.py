...
def sample_text_variant(
    labels: dict[str, list[str]],
    labels_free: dict[str, list[str]],
    combo: str,
    name_dropout_p: float,
) -> str | None:
    """Pick a random text variant for `combo`, dropping the species name with
    probability `name_dropout_p` (draw from the name-free pool instead).

    p=0 -> always name-ful (baseline); p=1 -> always name-free (masked);
    0<p<1 -> namedropout. Data budget is identical across p (same clips/steps);
    only the text-variant distribution changes, so a dropout-rate sweep isolates
    the effect of name-free variants from any dataset-size confound.
    """
    if (name_dropout_p > 0.0 and labels_free and combo in labels_free
            and random.random() < name_dropout_p):
        return random.choice(labels_free[combo])
    if combo in labels:
        return random.choice(labels[combo])
    return None
...
class ClapPairDataset(Dataset):
    ...
    def __init__(
        ...
        labels_namefree_path: Path | None = None,
        name_dropout_p: float = 0.0,
    ) -> None:
        ...
        # Optional name-free (masked) pool + per-clip name-dropout probability.
        self.labels_free: dict[str, list[str]] = (
            json.loads(labels_namefree_path.read_text(encoding="utf-8"))
            if labels_namefree_path and labels_namefree_path.exists() else {}
        )
        self.name_dropout_p = name_dropout_p
    ...
    def __getitem__(self, idx: int) -> dict[str, Any] | None:
        ...
        # Text augmentation: pick a label variant for this combo. With name-dropout
        # probability p, draw from the name-free pool instead (drops the species name).
        combo = pair.get("combo", "")
        text  = pair["text"]
        if self.augment and combo:
            picked = sample_text_variant(self.labels, self.labels_free, combo,
                                         self.name_dropout_p)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--labels-namefree",
        default=None,
        help="Path to a name-free (masked) labels file. With --name-dropout-p each "
             "clip draws its text from this pool with that probability. Enables the "
             "dropout-rate sweep (p=0 baseline names, p=1 fully masked).",
    )
    ap.add_argument(
        "--name-dropout-p",
        type=float,
        default=0.0,
        help="Per-clip probability of dropping the species name (draw text from "
             "--labels-namefree). 0.0 = name-ful baseline, 1.0 = full masking, "
             "0<p<1 = namedropout. (default 0.0)",
    )
    ap.add_argument(
        "--taxonomy",
...
def main(argv: list[str] | None = None) -> int:
    ...
    labels_free_path = (
        Path(args.labels_namefree)
        if args.labels_namefree and Path(args.labels_namefree).exists() else None
    )
    if args.name_dropout_p > 0 and labels_free_path:
        print(f"  Name-dropout: p={args.name_dropout_p} — name-free pool from {labels_free_path}")
    elif args.name_dropout_p > 0:
        print(f"  [warn] --name-dropout-p={args.name_dropout_p} set but no valid "
              f"--labels-namefree; name-dropout disabled")

    if use_precomputed:
        train_ds = ClapPrecomputedDataset(Path(args.train_pairs), audio_root,
                                          augment=do_augment, labels_path=labels_path,
                                          labels_namefree_path=labels_free_path,
                                          name_dropout_p=args.name_dropout_p)
